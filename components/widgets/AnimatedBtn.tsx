...
const AnimatedBtn: React.FC<ChildProps> = ({ children }) => {
  return (
    <button className="relative inline-flex h-10 overflow-hidden rounded-full p-[1px] px-[1.8px] focus:outline-none focus:ring-offset-2 focus:ring-offset-slate-50">
      ...
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-bgSecondary hover:bg-primary px-4 py-1 text-sm font-semibold text-primary hover:text-black backdrop-blur-3xl duration-300">
        {children}
      </span>
    </button>
  );
